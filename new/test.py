#!/usr/bin/env python3
"""
test.py – Batch Hint Recommender v4-c (debug-aware)
────────────────────────────────────────────────────
* Adaptive Embedding-Indexed Memory v2
* Model-Aware Thompson Bandit
* Guaranteed no-regression baseline
* Extra diagnostics:
    --debug           capture & warn on “bad‐hint” rows
    --dbg-thresh <T>  flag when actual_ms > pred_ms × T   (default 2)
Usage example:
    python test.py --sql-dir ./queries --out ./results --debug --dbg-thresh 1.5
"""
# ─── stdlib ───────────────────────────────────────────────────────────
...
try:
    for sql_file in sorted(RAW_SQL_DIR.rglob("*.sql")):
        ...
        # refresh / record baseline latency occasionally
